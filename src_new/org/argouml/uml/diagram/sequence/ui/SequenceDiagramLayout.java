...
public class SequenceDiagramLayout extends LayerPerspective {
    protected static Log logger = 
        org.apache.commons.logging.LogFactory.getLog(SequenceDiagramLayout.class);
  ...
  public void nodeAdded(GraphEvent ge) {
    ...
    if (null == oldDE) {
      if (!shouldShow(node)) { logger.debug("node rejected"); return; }
  ...
  public void edgeAdded(GraphEvent ge) {
    ...
    if (null == oldFig) {
      if (!shouldShow(edge)) { logger.debug("edge rejected"); return; }
  ...
  public void placeAllFigures() {
    ...
    for (i=0; i<linkSize; i++) {
      ...
      if (link.getSourcePortFig() == sourcePort._lifeline) {
        ...
        logger.debug("SeqDiagramLay.playceAll..: setSourcePortFig->DynPort");
