...
class SimpleStateTransformer implements Transformer {
    ...
    class SimpleStateToCompositeState extends TransformerAction {
        ...
        SimpleStateToCompositeState(Project project, Object sourceModelElement) {
            super("button.new-compositestate", project, sourceModelElement);
        }
...
//                ((LayerDiagram) editor.getLayerManager().getActiveLayer());
            ...
            Object entry = Model.getFacade().getEntry(source);
            Object exit = Model.getFacade().getExit(source);
            /* Need to copy the lists since we will be moving the elements: */
            Collection deferEvts = new ArrayList(Model.getFacade().getDeferrableEvents(source));
            Collection intTrans = new ArrayList(Model.getFacade().getInternalTransitions(source));
            Object doActivity = Model.getFacade().getDoActivity(source);
            Object sm = Model.getFacade().getStateMachine(source);
            Object container = Model.getFacade().getContainer(source);
            Collection outgoings = new ArrayList(Model.getFacade().getOutgoings(source));
            Collection incomings = new ArrayList(Model.getFacade().getIncomings(source));
            ...
            String name = Model.getFacade().getName(source);
            Object cs = Model.getStateMachinesFactory().buildCompositeState(container);
            Model.getStateMachinesHelper().setEntry(source, null);
            Model.getStateMachinesHelper().setEntry(cs, entry);
            Model.getStateMachinesHelper().setExit(source, null);
            Model.getStateMachinesHelper().setExit(cs, exit);
            for (Object de : deferEvts) {
                Model.getStateMachinesHelper().removeDeferrableEvent(source, de);
                Model.getStateMachinesHelper().addDeferrableEvent(cs, de);
            }
            Model.getStateMachinesHelper().setInternalTransitions(source, Collections.EMPTY_LIST);
            Model.getStateMachinesHelper().setInternalTransitions(cs, intTrans);
            Model.getStateMachinesHelper().setDoActivity(source, null);
